...
/*	This class can plot Multiple Lines on the same plot.
 *	It is based largely on the Plot class and extends its functionality towards
 *	displaying and saving the different line values
 *    
...
public class MultyPlotExt {
	...
	*/
	public MultyPlotExt(String title, String xLabel, String yLabel, float[] xValues, float Ymin, float Ymax) {
		this.title = title;
		this.xLabel = xLabel;
		this.yLabel = yLabel;
...
//		a = Tools.getMinMax(yValues);
	...
	public MultyPlotExt(String title, String xLabel, String yLabel, float[] xValues, float[] yValues) {
		this.title = title;
		this.xLabel = xLabel;
		this.yLabel = yLabel;
		this.xValues = xValues;
//		this.yValues = yValues;
	...
	public MultyPlotExt(String title, String xLabel, String yLabel, double[] xValues, double[] yValues) {
...
//	}
	...
	public float[][] getYValues() {
		...
		int i=0;
		for  (Enumeration e=yValues.elements();e.hasMoreElements();) {
//			ip.setColor(255);
			v[i] =(float[])e.nextElement();
			i++;
		}
		return v; 
	}

	public float[][] getErrorBars() {
		int sz=errorBars.size();
		float [][] v=new float[sz][];
		int i=0;
		for  (Enumeration e=errorBars.elements();e.hasMoreElements();) {
//			ip.setColor(255);
			v[i] =(float[])e.nextElement();
			i++;
		}
...
//	}
	...
	public void draw() {
		...
		float[] e;
		try {
			a = (float[]) yValues.firstElement();
//			e = (float[]) errorBars.firstElement();
		}
		catch (ClassCastException cce) {
			a = new float[xValues.length];
//			e = new float[xValues.length];
		}
		e = new float[xValues.length];

		for (int i=0; i<nPoints; i++) {
			xpoints[i] = LEFT_MARGIN + (int)((xValues[i]-xMin)*xScale);
			ypoints[i] = TOP_MARGIN + frame.height - (int)((a[i]-yMin)*yScale);
		}
		drawPolyline(ip, xpoints, ypoints, nPoints); 
		ip.setClipRect(null);					

//		if (this.errorBars != null) {
		if (!errorBars.isEmpty()) {
			e = (float[]) errorBars.firstElement();
			xpoints = new int[2];
			ypoints = new int[2];
			for (int i=0; i<nPoints; i++) {
				xpoints[0] = xpoints[1] = LEFT_MARGIN + (int)((xValues[i]-xMin)*xScale);
				ypoints[0] = TOP_MARGIN + frame.height - (int)((a[i]-yMin-e[i])*yScale);
				ypoints[1] = TOP_MARGIN + frame.height - (int)((a[i]-yMin+e[i])*yScale);
...
//		new ImagePlus("after", label).show();
	...
	public MultyPlotWindowExt show() {
		draw();
		return new MultyPlotWindowExt(this);
	}
